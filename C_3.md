...
Instead of calculating $P(C|X)$ directly, we compute $P(C|X) ∝ P(C) × ∏ᵢP(Xᵢ|C)$, where "$\large ∝$" means "proportional
to."
...
```mermaid
flowchart LR
    A["Full Formula: P(A|B) = P(B|A)P(A)/P(B)"] --> B["Proportional Form: P(A|B) ∝ P(B|A)P(A)"]
    style A fill:#BCFB89,stroke:#333,stroke-width:1px
    style B fill:#FBF266,stroke:#333,stroke-width:1px
