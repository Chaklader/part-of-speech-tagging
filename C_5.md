...
graph TD
    ...
    HMM --> Eval["1. Evaluation\nCalculate probability of\na word-tag sequence"]
    HMM --> Decode["2. Decoding\nFind most likely tag sequence\nfor a given word sequence"]
    HMM --> Learn["3. Learning\nEstimate model parameters\nfrom untagged data"]

    %% Evaluation details
    Eval --> EvalFormula["P(words, tags) = P(tag₁) × P(word₁|tag₁) ×\n∏ P(tagᵢ|tagᵢ₋₁) × P(wordᵢ|tagᵢ)"]
    Eval --> EvalUse["Use: Comparing likelihood\nof different analyses"]

    %% Decoding details
    Decode --> VitAlg[Viterbi Algorithm]
    Decode --> POSTask["POS Tagging Task\n'Mary will see Jane' → N-M-V-N"]
    VitAlg --> FindPath[Find optimal path through\nthe state lattice]

    %% Learning details
    Learn --> Supervised[Supervised: Count from\ntagged corpus]
    Learn --> Unsupervised[Unsupervised: Use\nBaum-Welch/EM Algorithm]
    Unsupervised --> EM[Expectation-Maximization:\nIteratively refine parameters]
