...
- **Mutually exclusive events** mean that if one happens, the other _cannot_ happen.

    Mathematically, $( A \cap B = \emptyset )$, so $( P(A \cap B) = 0 )$.

    The probability of _either_ occurring is: $[P(A \cup B) = P(A) + P(B)]$

    Example: Rolling a die, let \( A = \{1\} \) and \( B = \{2\} \). These are mutually exclusive because you can’t roll
    a 1 and a 2 on the same throw. So, $( P(1 \text{ or } 2) = \frac{1}{6} + \frac{1}{6} = \frac{1}{3} )$, and
    $( P(1 \text{ and } 2) = 0 )$.

- **Independent events** mean the occurrence of one doesn’t affect the probability of the other.

    Mathematically, $( P(A\cap B) = P(A) \cdot P(B) )$, and $( P(A|B) = P(A) )$.

    The probability of _both_ occurring is: $[P(A \cap B) = P(A) \cdot P(B)]$

    Example: Flipping a coin twice, let $( A = \{\text{heads on flip 1}\} )$ and $( B = \{\text{heads on flip 2}\} )$.
    These are independent because the first flip doesn’t change the second. So,
