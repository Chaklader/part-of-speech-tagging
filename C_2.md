...
- For the entire sample space Ω: $P(\Omega) = 1$
...
- Joint probability: $P(A \cap B) = P(A,B) = P(A|B) \times P(B) = P(B|A) \times P(A)$
- Marginal probability: $P(A) = \sum_{b \in B} P(A,b)$

###### Independence

- Events A and B are independent if: $P(A \cap B) = P(A) \times P(B)$
- Conditional independence: $A \perp B | C$ if $P(A|B,C) = P(A|C)$

###### Bayes' Theorem

- $P(A|B) = \frac{P(B|A) \times P(A)}{P(B)}$
- Expanded form: $P(A|B) = \frac{P(B|A) \times P(A)}{P(B|A) \times P(A) + P(B|\neg A) \times P(\neg A)}$

###### Law of Total Probability

- For a partition {B₁, B₂, ..., Bₙ}: $P(A) = \sum_{i=1}^{n} P(A|B_i) \times P(B_i)$
- Conditional form: $P(A|C) = P(A|B,C)P(B|C) + P(A|\neg B,C)P(\neg B|C)$

###### Chain Rule of Probability

- General form: $P(X_1, X_2, ..., X_n) = P(X_1) \times P(X_2|X_1) \times ... \times P(X_n|X_1, X_2, ..., X_{n-1})$
- For Bayesian networks: $P(X_1, X_2, ..., X_n) = \prod_{i=1}^{n} P(X_i | \text{Parents}(X_i))$

###### Normalizing Probabilities

- For unnormalized probabilities P': $P(A) = \frac{P'(A)}{\sum_{i} P'(A_i)}$
- In Bayes' theorem: $P(A|B) = \eta \cdot P(B|A)P(A)$ where $\eta = \frac{1}{P(B)}$
...
###### Parameter Counting in Bayesian Networks

- For a node with k states and parents with states s₁, s₂, ..., sₘ:
  $\text{Parameters} = (k-1) \cdot \prod_{i=1}^{m} s_i$

###### D-Separation Criteria

- A path is blocked by Z if it contains:
    - A chain X→Y→Z where Y is in Z
    - A fork X←Y→Z where Y is in Z
    - A collider X→Y←Z where neither Y nor any descendant of Y is in Z

###### Nature of the Events

In probability, the choice between addition and multiplication depends on the nature of the events:

- **Addition** is used for mutually exclusive events, where $( A \cap B = \emptyset )$ (the events cannot occur
  simultaneously). The probability of \( A \) _or_ \( B \) occurring is given by: $[P(A \cup B) = P(A) + P(B)]$ For
  example, if $( P(\text{rolling a 1}) = \frac{1}{6})$ and $( P(\text{rolling a 2}) = \frac{1}{6})$ on a die, then:
  $[P(\text{1 or 2}) = P(1) + P(2) = \frac{1}{6} + \frac{1}{6} = \frac{2}{6} = \frac{1}{3}]$
- **Multiplication** is used for independent events, where the occurrence of one event does not affect the other. The
  probability of \( A \) _and_ \( B \) occurring together is: $[P(A \cap B) = P(A) \cdot P(B)]$ For example, if
  $( P(\text{heads on flip 1}) = \frac{1}{2})$ and $( P(\text{heads on flip 2}) = \frac{1}{2} )$ for two coin flips,
  then:
  $[P(\text{heads and heads}) = P(\text{heads}) \cdot P(\text{heads}) = \frac{1}{2} \cdot \frac{1}{2} = \frac{1}{4}]$

If events are not mutually exclusive, you adjust with: $[P(A \cup B) = P(A) + P(B) - P(A \cap B)]$ And if events are not
independent, you use conditional probability: $[P(A \cap B) = P(A) \cdot P(B|A)]$

where \( P(B|A) \) is the probability of \( B \) given \( A \).

Mutually exclusive events and independent events are _not_ the same thing—in fact, they’re quite different, and that
distinction explains why we use addition for one and multiplication for the other.

- **Mutually exclusive events** mean that if one happens, the other _cannot_ happen. Mathematically,
  $( A \cap B = \emptyset )$, so $( P(A \cap B) = 0 )$. The probability of _either_ occurring is:
  $[P(A \cup B) = P(A) + P(B)]$ Example: Rolling a die, let \( A = \{1\} \) and \( B = \{2\} \). These are mutually
  exclusive because you can’t roll a 1 and a 2 on the same throw. So,
  $( P(1 \text{ or } 2) = \frac{1}{6} + \frac{1}{6} = \frac{1}{3} )$, and $( P(1 \text{ and } 2) = 0 )$.
- **Independent events** mean the occurrence of one doesn’t affect the probability of the other. Mathematically, \( P(A
  \cap B) = $P(A) \cdot P(B) ), and ( P(A|B) = P(A) )$. The probability of _both_ occurring is:
  $[P(A \cap B) = P(A) \cdot P(B)]$ Example: Flipping a coin twice, let $( A = \{\text{heads on flip 1}\} )$ and
  $( B = \{\text{heads on flip 2}\} )$. These are independent because the first flip doesn’t change the second. So,
  $( P(\text{heads and heads}) = \frac{1}{2} \cdot \frac{1}{2} = \frac{1}{4} )$.

Now, why aren’t mutually exclusive events independent? If \( A \) and \( B \) are mutually exclusive and both have
non-zero probabilities (\( P(A) > 0 \), \( P(B) > 0 \)), then knowing \( A \) happened tells you \( B \) didn’t—meaning
\( P(B|A) = 0 \), which doesn’t equal \( P(B) \). For independence, \( P(B|A) \) must equal \( P(B) \), but mutual
exclusivity forces $( P(A \cap B) = 0 )$, breaking the condition $( P(A \cap B) = P(A) \cdot P(B) )$ unless one event
has probability 0. For instance, if \( P(A) = 0.3 \) and \( P(B) = 0.4 \), independence would require
$( P(A \cap B) = 0.3 \cdot 0.4 = 0.12 )$, but mutual exclusivity demands $( P(A \cap B) = 0 )$, a contradiction.

So, mutually exclusive events (with non-zero probabilities) are _dependent_ by nature, not independent. That’s why we
add for "or" (mutually exclusive) and multiply for "and" (independent).
...
###### Understanding What Conditional Probability Means

When we write $P(A|B)$, we're asking: "Given that event B has occurred, what is the probability that event A occurs?"
This is indeed fundamentally different from asking about both events happening together.

The key insight is that conditional probability represents a restriction of our sample space. We're no longer
considering all possible outcomes - we're only looking at the subset of outcomes where B has occurred.

###### The Intuition Behind the Formula

Let's think about this visually:

- Imagine all possible outcomes as a large area
- The outcomes where event B occurs form a region within this area
- The outcomes where both A and B occur form a subregion within the B region

When we say "given B has occurred," we're essentially zooming in to only look at the B region as our new sample space.
Within this reduced space, we want to know what fraction corresponds to event A also occurring.
...
In probability terms, this translates directly to: $$P(A|B) = \frac{P(A \cap B)}{P(B)}$$

###### A Concrete Example

Imagine a standard deck of 52 playing cards:

- Let B = "drawing a heart" (13 cards)
- Let A = "drawing a face card" (12 cards: jacks, queens, kings)

The joint probability $P(A,B)$ = "drawing a card that is both a heart and a face card" = 3/52

The probability of B, $P(B)$ = 13/52

Now, if someone has already drawn a heart (event B), what's the probability it's a face card (event A)?

- Looking only at hearts, there are 13 cards total
- Of these, 3 are face cards (jack, queen, king of hearts)
- So $P(A|B) = 3/13$

Using our formula: $$P(A|B) = \frac{P(A,B)}{P(B)} = \frac{3/52}{13/52} = \frac{3}{13}$$

This gives us exactly the right answer!

###### Why This Makes Mathematical Sense

The formula $P(A|B) = \frac{P(A,B)}{P(B)}$ can be understood as a form of normalization:

- $P(A,B)$ counts the outcomes where both A and B occur
- But these counts are relative to the entire sample space
- Dividing by $P(B)$ rescales these counts to be relative to just the B-outcomes

This rescaling is necessary because conditional probability is defined within the restricted universe where B has
occurred.

###### Mathematical Proof

We can also derive this formula from first principles:

1. Let Ω be our sample space with probability measure P
2. Event B is a subset of Ω with P(B) > 0
3. For any event A, the conditional probability P(A|B) is a new probability measure on Ω
4. For this measure to be consistent with our intuition about conditioning:
    - Events outside B should have zero conditional probability
    - The total probability within B should be 1
    - The relative likelihoods of events within B should be preserved
5. This leads to the definition: $P(A|B) = \frac{P(A \cap B)}{P(B)}$

This formula ensures all our requirements are met and provides a mathematically sound foundation for conditional
probability.

###### Visualizing the Relationship Between Joint and Conditional Probability

Let me create a diagram to illustrate the relationship between joint probability P(A,B) and conditional probability
P(A|B):

```mermaid
graph TD
    subgraph "Complete Sample Space"
    Universe["All Possible Outcomes"] --- B_area["B occurs"]
    Universe --- notB["B does not occur"]
    B_area --- AnB["A and B both occur"]
    B_area --- BnotA["B occurs, A does not"]
    end

    style Universe fill:#BCFB89,stroke:#333,stroke-width:1px
    style B_area fill:#FBF266,stroke:#333,stroke-width:1px
    style notB fill:#FA756A,stroke:#333,stroke-width:1px
    style AnB fill:#9AE4F5,stroke:#333,stroke-width:1px
    style BnotA fill:#0096D9,stroke:#333,stroke-width:1px
```

To understand conditional probability P(A|B), imagine we "zoom in" on just the area where B occurs (the yellow region).
Within this restricted area, P(A|B) measures what fraction is also in event A (the light blue region).

Mathematically, this is:

$$P(A|B) = \frac{P(A \cap B)}{P(B)} = \frac{\text{Area of (A and B)}}{\text{Area of B}}$$

###### A Concrete Example with Playing Cards

Let's visualize this with our playing card example:

```mermaid
graph TD
    subgraph "52 Playing Cards"
    AllCards["All 52 Cards"] --- Hearts["Hearts (13 cards)"]
    AllCards --- NotHearts["Non-Hearts (39 cards)"]
    Hearts --- FaceHearts["Face Hearts (3 cards)"]
    Hearts --- NonFaceHearts["Non-Face Hearts (10 cards)"]
    NotHearts --- FaceNotHearts["Face Non-Hearts (9 cards)"]
    NotHearts --- NonFaceNotHearts["Non-Face Non-Hearts (30 cards)"]
    end

    style AllCards fill:#BCFB89,stroke:#333,stroke-width:1px
    style Hearts fill:#FBF266,stroke:#333,stroke-width:1px
    style NotHearts fill:#FA756A,stroke:#333,stroke-width:1px
    style FaceHearts fill:#9AE4F5,stroke:#333,stroke-width:1px
    style NonFaceHearts fill:#0096D9,stroke:#333,stroke-width:1px
    style FaceNotHearts fill:#FCEB14,stroke:#333,stroke-width:1px
    style NonFaceNotHearts fill:#FE9237,stroke:#333,stroke-width:1px
```

If we define:

- B = "drawing a heart" (13 cards)
- A = "drawing a face card" (12 cards: 3 in hearts, 9 in other suits)

Then:

- P(B) = 13/52 = 1/4
- P(A,B) = P(A∩B) = 3/52
- P(A|B) = P(A,B)/P(B) = (3/52)/(13/52) = 3/13

The probability of drawing a face card, given that we've drawn a heart, is 3/13. This makes sense because among the 13
hearts, exactly 3 are face cards.

###### An Alternative Visual: Venn Diagram Perspective

If Mermaid supported proper Venn diagrams, we could visualize it this way:

```mermaid
graph TD
    subgraph "Probability Space"
    VD["<br><br><br><br><br><br><br><br><br>"]
    A_label["A: Face Cards (12/52)"] --- VD
    B_label["B: Hearts (13/52)"] --- VD
    AB_label["A∩B: Face Hearts (3/52)"] --- VD
    end

    style VD fill:#BCFB89,stroke:#333,stroke-width:1px
    style A_label fill:white,stroke:none
    style B_label fill:white,stroke:none
    style AB_label fill:white,stroke:none
```

When calculating P(A|B), we're restricting our attention to just the region B and asking what fraction of this region
also falls in region A.

###### Why This Formula Makes Sense

The formula P(A|B) = P(A,B)/P(B) makes intuitive sense because:

1. The numerator P(A,B) counts outcomes where both A and B occur
2. The denominator P(B) counts all outcomes where B occurs
3. The ratio gives us the proportion of B-outcomes that also satisfy A

This is exactly what conditional probability represents: within the restricted universe where B has occurred, what's the
probability that A also occurs?

The apparent "inverse" relationship you noted isn't really inverse - it's a rescaling or normalization process. We're
taking joint probability (which is measured relative to the entire sample space) and rescaling it to be measured
relative to just the event B.

###### Understanding Joint Probability Notation

When we see the notation $P(A \cap B)$ or equivalently $P(A, B)$, we're looking at what's called the joint probability
of events A and B. Let me explain what this means and how it relates to conditional probability.

###### Joint Probability: $P(A \cap B)$ or $P(A, B)$

The joint probability $P(A \cap B)$ represents the probability that both event A and event B occur simultaneously. The
symbol "$\cap$" is the set intersection operator, indicating "and" in probability language.

These two notations are equivalent:

- $P(A \cap B)$ - using the intersection symbol
- $P(A, B)$ - using the comma notation

Both represent exactly the same concept: the probability of A and B happening together.

###### Relationship to Conditional Probability

The multiplication rule connects joint probability with conditional probability:

$$P(A \cap B) = P(A|B) \times P(B)$$

This formula reveals something profound about joint probability:

- We can think of it as first requiring event B to happen (with probability $P(B)$)
- And then, given that B has happened, event A must also happen (with probability $P(A|B)$)

The probability of this entire sequence is the product of these two probabilities.

###### Example to Illustrate the Concept

Imagine we have a deck of 52 playing cards. Let's define:

- Event A: Drawing a face card (King, Queen, or Jack) = 12 cards
- Event B: Drawing a heart = 13 cards

The joint probability $P(A \cap B)$ means drawing a card that is both a face card AND a heart.

- $P(A \cap B) = P(\text{face card and heart}) = 3/52$

We can verify this using the multiplication rule:

- $P(B) = 13/52 = 1/4$ (probability of drawing a heart)
- $P(A|B) = 3/13$ (probability of drawing a face card, given that we've drawn a heart)
- $P(A \cap B) = P(A|B) \times P(B) = (3/13) \times (1/4) = 3/52$

###### Symmetry of Joint Probability

Another important property is that joint probability is symmetric:
...
- $P(A) = 12/52 = 3/13$ (probability of drawing a face card)
- $P(B|A) = 3/12 = 1/4$ (probability of drawing a heart, given that we've drawn a face card)
- $P(A \cap B) = P(B|A) \times P(A) = (1/4) \times (3/13) = 3/52$

###### Visual Representation

Think of the sample space as a rectangle divided into four regions:

1. A and B both occur (A ∩ B)
2. A occurs but B doesn't (A ∩ ¬B)
3. B occurs but A doesn't (¬A ∩ B)
4. Neither A nor B occurs (¬A ∩ ¬B)

The joint probability $P(A \cap B)$ represents the proportion of outcomes in region 1.
...
For three events A, B, and C, the probability to happen A given B and C happened:
...
- People without disease who tested positive or the false positive rate is 10% (P(T|not D) = 0.10)
...
$P(T) = P(T|D) × P(D) + P(T|not\;\text{D}) × P(not_\;\text{D}) = 0.95 × 0.01 + 0.10 × 0.99 = 0.0095 + 0.099 = 0.1085$

Therefore: P(D|T) = 0.95 × 0.01 / 0.1085 ≈ 0.088 or about 8.8%

###### Analyzing the Medical Test Scenario

Given information:

- Disease prevalence: $P(D) = 0.01$ (1% of population has the disease)
- Test sensitivity: $P(T|D) = 0.95$ (95% of people with disease test positive)
- False positive rate: $P(T|\neg D) = 0.10$ (10% of people without disease test positive)

The calculation for $P(D|T)$ using Bayes' theorem is:
...
Therefore: $$P(D|T) = \frac{0.95 \times 0.01}{0.1085} \approx 0.088 \approx 8.8%$$

###### Addressing Your Concern About False Positive Rate

You mentioned that the false positive rate might need to be 5% instead of 10%. Let's examine this question:

If the test is "95% accurate," this could mean different things:

1. The test correctly identifies 95% of people with the disease (sensitivity = 95%)
2. The test correctly identifies 95% of people without the disease (specificity = 95%)
3. The test gives the correct result 95% of the time overall (total accuracy = 95%)

In this problem:

- We're explicitly told the sensitivity is 95%: $P(T|D) = 0.95$
- We're explicitly told the false positive rate is 10%: $P(T|\neg D) = 0.10$

The false positive rate and specificity are related: Specificity = 1 - False positive rate

- So specificity = 1 - 0.10 = 0.90 or 90%

This means the test correctly identifies:

- 95% of sick people as sick
- 90% of healthy people as healthy

With these parameters, the calculation of 8.8% is mathematically correct.

If we were to change the false positive rate to 5% (specificity = 95%), the calculation would be:
$$P(T) = 0.95 \times 0.01 + 0.05 \times 0.99 = 0.0095 + 0.0495 = 0.059$$
$$P(D|T) = \frac{0.95 \times 0.01}{0.059} \approx 0.161 \approx 16.1%$$
...
3. Since these intersections are mutually exclusive, we can add their probabilities:
   $\begin{align}P(A) &= P(A \cap B_1) + P(A \cap B_2) + ... + P(A \cap B_n) \\& = P(A , B_1) + P(A , B_2) + ... + P(A , B_n)\end{align}$
...
$$P(\text{Fever}) = P(\text{Fever}|\text{Infection}) \times P(\text{Infection}) + P(\text{Fever}|\neg \text{Infection}) \times P(\neg \text{Infection})$$
...
###### Understanding Partitions in the Law of Total Probability

The term "partition" in probability theory refers to a way of dividing the sample space into mutually exclusive and
collectively exhaustive sets. Let me explain this concept more precisely.

###### Mathematical Definition of a Partition

A partition of a sample space Ω is a collection of subsets {B₁, B₂, ..., Bₙ} such that:

1. Every subset is non-empty: Bᵢ ≠ ∅ for all i
2. The subsets are mutually exclusive: Bᵢ ∩ Bⱼ = ∅ for all i ≠ j
3. The subsets cover the entire sample space: B₁ ∪ B₂ ∪ ... ∪ Bₙ = Ω

In simpler terms, a partition splits the sample space into non-overlapping pieces that together constitute the entire
space.

###### In the Law of Total Probability

In the context of the conditional form of the Law of Total Probability:

$$P(A|C) = P(A|B,C)P(B|C) + P(A|\bar{B},C)P(\bar{B}|C)$$

Here, {B, B̄} forms a partition because:

- B and B̄ are mutually exclusive (an event cannot be both B and not B)
- B and B̄ are collectively exhaustive (every outcome must be either B or not B)

This partition allows us to break down the calculation of P(A|C) by considering two distinct scenarios:

1. When B occurs (along with C)
2. When B does not occur (along with C)

###### Practical Example

Consider calculating the probability of having a fever (F) given that it's winter (W), where we partition based on
whether a person has the flu (I) or not having the flu (Ī):

$$P(F|W) = P(F|I,W)P(I|W) + P(F|Ī,W)P(Ī|W)$$

This allows us to calculate the probability by considering:

- How likely fever is among winter flu patients: $P(F|I,W)$
- How likely fever is among winter non-flu patients: $P(F|Ī,W)$
- The prevalence of flu in winter: $P(I|W)$ and $P(Ī|W)$ where $P(Ī|W) = 1-P(I|W)$

###### Example with Hypothetical Values

If we assign some values:

- $P(F|I,W) = 0.8$ (80% of people with flu in winter develop fever)
- $P(F|Ī,W) = 0.1$ (10% of people without flu in winter develop fever)
- $P(I|W) = 0.2$ (20% of people get flu in winter)

Then:

- $P(Ī|W) = 1-0.2 = 0.8$ (80% of people don't get flu in winter)

Substituting these values: $$P(F|W) = 0.8 \times 0.2 + 0.1 \times 0.8 = 0.16 + 0.08 = 0.24$$

This tells us that about 24% of people develop a fever during winter, with the calculation breaking this down into the
two possible pathways through which fever can occur in winter - either with or without flu.

###### Generalized Form

The formula generalizes to any partition $\{B₁, B₂, ..., Bₙ\}$:

$$P(A|C) = \sum_{i=1}^{n} P(A|B_i,C)P(B_i|C)$$

This powerful formula allows us to compute conditional probabilities by breaking them down across different scenarios
defined by the partition, even when we don't have direct access to P(A|C).
...
- $\{C, \neg C\}$ form a partition of the sample space meaning:
...
3. Substituting into our original equation given, $P(A∩B)=P(A∣B)×P(B)$
...
5. Substituting back and simplifying:
    ...
    $\begin{align}&P(A|B) = [P(A|B,C)P(C|B)P(B) + P(A|B,¬C)P(¬C|B)P(B)]/P(B) \\ &P(A|B) =
   P(A|C,B)P(C|B) + P(A|¬C,B)P(¬C|B)\end{align}$
...
###### Chain Rule and Conditional Independence in Bayesian Networks

The chain rule of probability allows us to decompose a joint probability distribution into a product of conditional
probabilities. Let me explain how the simplifications work in the examples you provided.

###### The General Chain Rule

The general chain rule for three variables is:

$$P(A,B,C) = P(A|B,C) \times P(B|C) \times P(C)$$

This formula is always valid and makes no assumptions about independence. It simply breaks down the joint probability
into sequential conditional probabilities.

###### Simplification Through Conditional Independence

When conditional independence exists, we can simplify this formula. If A is conditionally independent of C given B
(denoted as A ⊥ C | B), then:

$$P(A|B,C) = P(A|B)$$

This means that once we know B, learning C provides no additional information about A. Mathematically, this simplifies
our chain rule to:

$$P(A,B,C) = P(A|B) \times P(B|C) \times P(C)$$

This simplification reduces the number of parameters we need to specify and makes calculations more efficient. Instead
of needing a conditional probability table for P(A|B,C) with entries for every combination of B and C values, we only
need the smaller table for P(A|B).

###### The Wet Grass Example

Let's analyze how this works in the wet grass example. Here we have four variables:
...
The Bayesian network structure implies certain conditional independence relationships:
...
graph TD
    ...
    style S fill:#FA756A,stroke:#333,stroke-width:1px
    style W fill:#9AE4F5,stroke:#333,stroke-width:1px
```

Using the general chain rule, we could write:

$$P(C,R,S,W) = P(W|R,S,C) \times P(S|R,C) \times P(R|C) \times P(C)$$

###### The Foundation: What is Conditional Probability?

Let's start with just two events, A and B. The conditional probability formula tells us:
...
This is telling us that the probability of both A and B happening equals the probability of B happening, multiplied by
the probability that A happens given that B has happened.

Think of it like this: to find the probability that you're both wearing a red shirt AND it's raining, you could
multiply:

1. The probability that it's raining
2. The probability you wear a red shirt when it rains

###### Extending to Three Variables

Now let's add a third variable C. How do we find P(A,B,C) - the probability that A, B, and C all happen?

We can use our formula from before, treating (A,B) as a joint event:

$$P(A,B,C) = P((A,B)|C) \times P(C)$$

But what is P((A,B)|C)? This is the probability that both A and B occur, given that C has occurred. We can apply our
formula again:
...
This is the Chain Rule for three variables. It tells us that the joint probability equals:

1. The probability of C
2. The probability of B given C
3. The probability of A given both B and C

###### Extending to Four Variables

For our wet grass example with four variables (C, R, S, W), we follow exactly the same pattern, adding one variable at a
time:

$$P(C,R,S,W) = P(W|C,R,S) \times P(C,R,S)$$

Now we apply the Chain Rule to P(C,R,S):

$$P(C,R,S) = P(S|C,R) \times P(C,R)$$

And again to P(C,R):

$$P(C,R) = P(R|C) \times P(C)$$

Substituting these expressions back:

$$P(C,R,S,W) = P(W|C,R,S) \times P(S|C,R) \times P(R|C) \times P(C)$$

###### A Concrete Example

Imagine we're tracking these events for a single day:

- C: It's cloudy
- R: It rains
- S: The sprinkler runs
- W: The grass gets wet

To find the probability of a specific scenario (say, it's cloudy, it rains, the sprinkler doesn't run, and the grass
gets wet), we need P(C=true, R=true, S=false, W=true).

Using the Chain Rule, we break this down into sequential questions:

1. What's the probability it's cloudy? P(C=true)
2. Given it's cloudy, what's the probability it rains? P(R=true|C=true)
3. Given it's cloudy and raining, what's the probability the sprinkler doesn't run? P(S=false|C=true,R=true)
4. Given it's cloudy, raining, and the sprinkler isn't running, what's the probability the grass gets wet?
   P(W=true|C=true,R=true,S=false)

Multiplying these probabilities gives us the joint probability of that specific scenario.

The Chain Rule simply formalizes this intuitive way of breaking down complex events into a sequence of simpler
conditional probabilities. It's always valid, regardless of any independence relationships between the variables.

###### Understanding P(C, R, S, W) in the Wet Grass Example

P(C, R, S, W) represents the joint probability distribution over all four random variables in our wet grass scenario.
This is a fundamental concept in probability theory that deserves careful explanation.

The joint probability P(C, R, S, W) gives us the probability of a specific combination of values for all four variables
occurring together. For instance, it could represent the probability that it's cloudy AND raining AND the sprinkler is
on AND the grass is wet.

Since each variable is binary (true or false), there are 2⁴ = 16 possible combinations of values these variables could
take. The joint probability distribution assigns a probability to each of these 16 possibilities, with the constraint
that these probabilities must sum to 1.

Without any simplifying assumptions, specifying this joint distribution would require listing 15 independent parameters
(the 16th is determined by the constraint that probabilities must sum to 1). However, using the Bayesian network
structure and the conditional independence relationships it encodes, we can represent this joint distribution much more
compactly.

The factorization we derived:

$$P(C, R, S, W) = P(C) \times P(R|C) \times P(S|C) \times P(W|R,S)$$

allows us to specify the joint distribution using only:

- 1 parameter for P(C)
- 2 parameters for P(R|C)
- 2 parameters for P(S|C)
- 4 parameters for P(W|R,S)

That's a total of 9 parameters instead of 15, a significant reduction in complexity.

Once we have these parameters specified, we can calculate the probability of any specific combination of values. For
example, if we want to know P(C=true, R=true, S=false, W=true), we would compute:

$$P(C=\text{true}) \times P(R=\text{true}|C=\text{true}) \times P(S=\text{false}|C=\text{true}) \times P(W=\text{true}|R=\text{true},S=\text{false})$$

This joint probability distribution is powerful because it allows us to answer any probabilistic query about these
variables through marginalization (summing out irrelevant variables) and conditioning (applying Bayes' rule).

The beauty of the Bayesian network representation is that it makes explicit which variables directly influence others,
allowing us to exploit conditional independence to represent this complex distribution efficiently.

However, the network structure allows us to make two simplifications:

1. **First simplification**: W is conditionally independent of C given R and S.

    This means $P(W|R,S,C) = P(W|R,S)$

    Intuitively, once we know whether it's raining and whether the sprinkler is on, knowing if it's cloudy gives us no
    additional information about whether the grass is wet.

2. **Second simplification**: R and S are conditionally independent given C.

    This means $P(S|R,C) = P(S|C)$

    Intuitively, once we know whether it's cloudy, knowing if it's raining doesn't give us additional information about
    whether the sprinkler is on.

Applying these simplifications to our chain rule:

$$P(C,R,S,W) = P(W|R,S) \times P(S|C) \times P(R|C) \times P(C)$$

This is exactly the formula you provided:

$$P(\text{Cloudy}, \text{Rain}, \text{Sprinkler}, \text{Wet}) = P(\text{Cloudy}) \times P(\text{Rain}|\text{Cloudy}) \times P(\text{Sprinkler}|\text{Cloudy}) \times P(\text{Wet}|\text{Rain}, \text{Sprinkler})$$

###### Why These Simplifications Matter

These simplifications are crucial because they:

1. **Reduce parameter complexity**: Instead of specifying $P(W|R,S,C)$ which would require parameters for every
   combination of R, S, and C, we only need to specify $P(W|R,S)$.
2. **Enable efficient inference**: Algorithms for answering probability queries can exploit these independence
   relationships to perform calculations more efficiently.
3. **Make the model more interpretable**: The simplified structure more clearly represents our understanding of how
   these variables causally influence each other.
4. **Improve learning from data**: With fewer parameters to estimate, we can learn more robust probability distributions
   from limited data.

In the wet grass example, without these simplifications, we would need to specify 2³ = 8 parameters for $P(W|R,S,C)$
(assuming binary variables). With the simplification, we only need 2² = 4 parameters for $P(W|R,S)$. Similar savings
occur for the other conditional probability tables.

This demonstrates how identifying and leveraging conditional independence relationships is at the heart of what makes
Bayesian networks powerful for representing complex probability distributions efficiently.
...
$$\eta \cdot P'(A|B) + \eta \cdot P'(¬A|B) = 1$$

$$\eta \cdot [P'(A|B) + P'(¬A|B)] = 1$$
...
graph TD
    ...
    style Event fill:#BCFB89,stroke:#333,stroke-width:1px
    style A fill:#FBF266,stroke:#333,stroke-width:1px
    style B fill:#FA756A,stroke:#333,stroke-width:1px
    style R1 fill:#9AE4F5,stroke:#333,stroke-width:1px
    style R2 fill:#9AE4F5,stroke:#333,stroke-width:1px
    style NR1 fill:#0096D9,stroke:#333,stroke-width:1px
    style NR2 fill:#0096D9,stroke:#333,stroke-width:1px
