...
- Conditional independence:
    $$
    A \perp B | C \text{ if } P(A|B,C) = P(A|C)
    $$
...
- For unnormalized probabilities P':
    $$
    P(A) = \frac{P'(A)}{\sum_{i} P'(A_i)}
    $$
- In Bayes' theorem:
    $$
    P(A|B) = \eta \cdot P(B|A)P(A)
    $$
    where $\eta = \frac{1}{P(B)}$
...
- For a node with k states and parents with states $s_1, s_2, \ldots, s_m$:
    $$
...
- A path is blocked by $Z$ if it contains:
    - A chain $X \rightarrow Y \rightarrow Z$ where $Y$ is in $Z$
    - A fork $X \leftarrow Y \rightarrow Z$ where $Y$ is in $Z$
    - A collider $X \rightarrow Y \leftarrow Z$ where neither $Y$ nor any descendant of $Y$ is in $Z$
...
- **Addition** is used for mutually exclusive events, where $A \cap B = \emptyset$ (the events cannot occur
  simultaneously). The probability of $A$ _or_ $B$ occurring is given by: $$P(A \cup B) = P(A) + P(B)$$ For example, if
  $P(\text{rolling a 1}) = \frac{1}{6}$ and $P(\text{rolling a 2}) = \frac{1}{6}$ on a die, then:
...
- **Multiplication** is used for independent events, where the occurrence of one event does not affect the other. The
  ...
  $P(\text{heads on flip 1}) = \frac{1}{2}$ and $P(\text{heads on flip 2}) = \frac{1}{2}$ for two coin flips, then:
...
- **Mutually exclusive events** mean that if one happens, the other _cannot_ happen. Mathematically,
  $A \cap B = \emptyset$, so $P(A \cap B) = 0$.

The probability of _either_ occurring is: $$P(A \cup B) = P(A) + P(B)$$

Example: Rolling a die, let $A = \{1\}$ and $B = \{2\}$. These are mutually exclusive because you can't roll a 1 and a 2
on the same throw. So, $P(1 \text{ or } 2) = \frac{1}{6} + \frac{1}{6} = \frac{1}{3}$, and $P(1 \text{ and } 2) = 0$.
...
Example: Flipping a coin twice, let $A = \{\text{heads on flip 1}\}$ and $B = \{\text{heads on flip 2}\}$. These are
independent because the first flip doesn't change the second. So,
$P(\text{heads and heads}) = \frac{1}{2} \cdot \frac{1}{2} = \frac{1}{4}$.

Now, why aren't mutually exclusive events independent? If $A$ and $B$ are mutually exclusive and both have non-zero
probabilities ($P(A) > 0$, $P(B) > 0$), then knowing $A$ happened tells you $B$ didn't—meaning $P(B|A) = 0$, which
doesn't equal $P(B)$. For independence, $P(B|A)$ must equal $P(B)$, but mutual exclusivity forces $P(A \cap B) = 0$,
breaking the condition $P(A \cap B) = P(A) \cdot P(B)$ unless one event has probability 0. For instance, if $P(A) = 0.3$
and $P(B) = 0.4$, independence would require $P(A \cap B) = 0.3 \cdot 0.4 = 0.12$, but mutual exclusivity demands
$P(A \cap B) = 0$, a contradiction.
...
In probability terms, this translates directly to:
...
$$
P(A|B) = \frac{P(A,B)}{P(B)} = \frac{3/52}{13/52} = \frac{3}{13}
$$
