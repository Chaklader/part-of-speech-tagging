...
1. **Serial (Chain) Connection**: $X \rightarrow Z \rightarrow Y$
...
2. **Diverging (Fork) Connection**: $X \leftarrow Z \rightarrow Y$
...
3. **Converging (V-structure/Collider) Connection**: $X \rightarrow Z \leftarrow Y$
...
###### Why $P(J \mid A, B, E) = P(J \mid A)$ Holds

Returning to our original equation, we can now explain why $P(J \mid A, B, E) = P(J \mid A)$ holds through d-separation:
...
Therefore, J is conditionally independent of both B and E given A, which is precisely what the equation
$P(J \mid A, B, E) = P(J \mid A)$ states.
...
- $P(\text{Cloudy}=\text{Yes}) = 0.5$
- $P(\text{Cloudy}=\text{No}) = 0.5$

**$P(\text{Sprinkler}|\text{Cloudy})$:**

- $P(\text{Sprinkler}=\text{On} | \text{Cloudy}=\text{Yes}) = 0.1$
- $P(\text{Sprinkler}=\text{On} | \text{Cloudy}=\text{No}) = 0.5$

**$P(\text{Rain}|\text{Cloudy})$:**

- $P(\text{Rain}=\text{Yes} | \text{Cloudy}=\text{Yes}) = 0.8$
- $P(\text{Rain}=\text{Yes} | \text{Cloudy}=\text{No}) = 0.2$

**$P(\text{WetGrass} | \text{Sprinkler}, \text{Rain})$:**

- $P(\text{WetGrass}=\text{Yes} | \text{Sprinkler}=\text{On}, \text{Rain}=\text{Yes}) = 0.99$
- $P(\text{WetGrass}=\text{Yes} | \text{Sprinkler}=\text{On}, \text{Rain}=\text{No}) = 0.9$
- $P(\text{WetGrass}=\text{Yes} | \text{Sprinkler}=\text{Off}, \text{Rain}=\text{Yes}) = 0.9$
- $P(\text{WetGrass}=\text{Yes} | \text{Sprinkler}=\text{Off}, \text{Rain}=\text{No}) = 0.01$
...
1. **$P(\text{Cloudy})$**:
    - $P(C=\text{yes}) = 0.5$
    - $P(C=\text{no}) = 0.5$
2. **$P(\text{Sprinkler} | \text{Cloudy})$**:
    - $P(S=\text{on} | C=\text{yes}) = 0.1$
    - $P(S=\text{on} | C=\text{no}) = 0.5$
    - $P(S=\text{off} | C=\text{yes}) = 0.9$
    - $P(S=\text{off} | C=\text{no}) = 0.5$
3. **$P(\text{Rain} | \text{Cloudy})$**:
    - $P(R=\text{yes} | C=\text{yes}) = 0.8$
    - $P(R=\text{yes} | C=\text{no}) = 0.2$
    - $P(R=\text{no} | C=\text{yes}) = 0.2$
    - $P(R=\text{no} | C=\text{no}) = 0.8$
4. **$P(\text{WetGrass} | \text{Sprinkler}, \text{Rain})$**:
    - $P(W=\text{yes} | S=\text{on}, R=\text{yes}) = 0.99$
    - $P(W=\text{yes} | S=\text{on}, R=\text{no}) = 0.9$
    - $P(W=\text{yes} | S=\text{off}, R=\text{yes}) = 0.9$
    - $P(W=\text{yes} | S=\text{off}, R=\text{no}) = 0.01$
    - $P(W=\text{no} | S=\text{on}, R=\text{yes}) = 0.01$
    - $P(W=\text{no} | S=\text{on}, R=\text{no}) = 0.1$
    - $P(W=\text{no} | S=\text{off}, R=\text{yes}) = 0.1$
    - $P(W=\text{no} | S=\text{off}, R=\text{no}) = 0.99$

###### Demonstrating the Explaining Away Effect

To show the explaining away effect, we need to compare:

1. $P(\text{Sprinkler}=\text{on} | \text{WetGrass}=\text{yes})$ - Probability the sprinkler was on, given that the grass
   is wet
2. $P(\text{Sprinkler}=\text{on} | \text{WetGrass}=\text{yes}, \text{Rain}=\text{yes})$ - Probability the sprinkler was
   on, given that the grass is wet AND it rained
...
$$
P(R=\text{yes} | S=\text{on}) = \frac{0.09}{0.3} = 0.3
$$

And:

$$
P(R=\text{no} | S=\text{on}) = 1 - 0.3 = 0.7
$$
...
$$
P(R=\text{yes} | S=\text{off}) = \frac{0.41}{0.7} \approx 0.586
$$

And:

$$
P(R=\text{no} | S=\text{off}) = 1 - 0.586 = 0.414
$$
...
$$
P(W=\text{yes}) = 0.927 \cdot 0.3 + 0.531 \cdot 0.7 = 0.278 + 0.372 = 0.65
$$
...
$$
P(S=\text{on} | R=\text{yes}) = \frac{0.3 \cdot 0.3}{0.5} = 0.18
$$
...
$$
P(W=\text{yes} | R=\text{yes}) = 0.99 \cdot 0.18 + 0.9 \cdot 0.82 = 0.178 + 0.738 = 0.916
$$
